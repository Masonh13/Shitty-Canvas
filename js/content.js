...
async function getCards(api = null) {
    ...
    console.log("suck my penis vcock and balls and potentially even penis")
...
function loadCustomFont() {
    ...
    let load = () => {
        ...
        if (options.custom_font.link !== "") {
            ...
            link.href = options.custom_font.link;
            link.rel = "stylesheet";
            document.head.appendChild(link);
        }
        if (options.custom_font.format !== "")
            style.textContent += options.custom_font.link === "" ? "" : `@font-face { font-family: \"${options.custom_font.family}\"; src: URL('${options.custom_font.link}') format('${options.custom_font.format}'); } `;
...
function customizeCards(c = null) {
    ...
    try {
        ...
        cards.forEach(card => {
            let cardOptions = options["custom_cards"][(card.querySelector(".ic-DashboardCard__link").href.split("courses/")[1]).split("/")[0]];
            let cardOptions_2 = options["custom_cards_2"][(card.querySelector(".ic-DashboardCard__link").href.split("courses/")[1]).split("/")[0]];
            if (cardOptions) {
